...
#         image.to_filename(os.path.join(output_dir, "{0}".format(modality)))
#
...
#
#
# def run_validation_cases(validation_keys_file, model_file, training_modalities, labels, hdf5_file, output_label_map=False, output_dir=".", threshold=0.5, overlap=16, permute=False):
...
if __name__ == "__main__":
    ...
    print('load data')
    data = list()
    t1 = nib.load('../brats_comb_up/prediction/Brats18_CBICA_ABB_1/data_t1.nii.gz');
    affine = t1.affine
    data.append(t1.get_data())
    data.append(nib.load('../brats_comb_up/prediction/Brats18_CBICA_ABB_1/data_t1ce.nii.gz').get_data())
    data.append(nib.load('../brats_comb_up/prediction/Brats18_CBICA_ABB_1/data_flair.nii.gz').get_data())
    data.append(nib.load('../brats_comb_up/prediction/Brats18_CBICA_ABB_1/data_t2.nii.gz').get_data())
    data.append(nib.load('../brats_comb_up/prediction/Brats18_CBICA_ABB_1/up.nii').get_data())
    print('data loaded', time.time() - start)

    # predict and save:
    predict_one(model, data, affine)
